...
import { notifyQuotationSent } from '@/lib/warehouseNotifications';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log('🔔 Attempting to send quotation submission email...');
    try {
      ...
      // Get warehouse from PO or RFQ
      if (purchaseOrderId) {
        console.log(`Looking up warehouse from PO: ${purchaseOrderId}`);
        const poResult = await erpDb.execute(sql`
          SELECT po.po_number, po.warehouse_id, w.name as warehouse_name, w.email, w.manager_email
        ...
        console.log('PO Details:', {
          ...
          warehouse_email: po?.email,
          manager_email: po?.manager_email,
        });
        
        if (po && po.warehouse_id) {
          console.log(`Calling notifyQuotationSent for PO ${po.po_number}`);
          const result = await notifyQuotationSent(
            po.warehouse_id,
            po.po_number || 'N/A',
            supplierName
          );
      ...
      console.error('❌ Error sending quotation submission email:', emailError);
      console.error('Error sending quotation email notification:', emailError);
