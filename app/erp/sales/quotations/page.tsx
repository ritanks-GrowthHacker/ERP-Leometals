...
import EditSalesQuotationModal from '@/components/modal/EditSalesQuotationModal';
...
export default function QuotationsPage() {
  ...
  const fetchAllQuotationsForStats = async () => {
    ...
    try {
      const response = await fetch('/api/erp/sales/quotations?limit=1000', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSendQuotation = async (quotationId: string, quotationNumber: string) => {
    showConfirm({
      ...
      onConfirm: async () => {
        ...
        const token = getAuthToken();
        try {
          const response = await fetch(`/api/erp/sales/quotations/${quotationId}/send`, {
            method: 'POST',
            headers: {
